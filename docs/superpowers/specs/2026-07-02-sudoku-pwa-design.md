...
| Обновление приложения | **`registerType: 'prompt'`** + всплывающий баннер, который сворачивается в постоянную кнопку «Обновить» |
| Жизни | **3 жизни.** Ошибка = поставленная цифра ≠ `solution`. Каждая неверная постановка снимает жизнь; 0 жизней → партия проиграна (`outcome: 'lost'`) |
...
interface GameState {
  ...
  lives: number;                      // старт 3; -1 за каждую неверную постановку; 0 = проигрыш
  elapsedSeconds: number;
  startedAt: string;                  // ISO
  status: 'in_progress' | 'completed';
  result?: 'won' | 'lost';            // заполняется при переходе в 'completed'
}
...
interface Move {
  ...
  newValue: number;
  wasNote: boolean;
  wasMistake: boolean;                // newValue ≠ solution — эта постановка сняла жизнь
...
`CompletedGame` пишется в трёх точках:
- при победе (`isSolved`) → `outcome: 'won'`;
- при потере последней жизни → `outcome: 'lost'`;
- при старте новой игры, если текущая была `in_progress` → `outcome: 'abandoned'` (нужно для метрики «% завершённых»).

Метрика «% завершённых» = `won / (won + lost + abandoned)`.
...
2. `currentGrid[row][col] = newValue` (неверная цифра тоже вписывается — её видно и можно перезаписать).
3. **Проверка ошибки:** `wasMistake = newValue !== solution[row][col]`. Если ошибка — `lives -= 1`, показать вспышку/анимацию потери жизни. Клетка подсвечивается как ошибочная.
4. **Автоочистка:** убрать эту цифру из `notes` всех клеток той же строки, столбца, блока 3×3; собрать `clearedNotes`.
5. Сформировать `Move` (`prevValue`, `wasMistake`, `clearedNotes`) → push в `history`.
6. Пересчитать конфликты через `validator.findConflicts` (дёшево, без solver).
7. **Проигрыш:** если `lives === 0` → `status = 'completed'`, `result = 'lost'`, записать `CompletedGame(lost)`, показать `WinScreen` в режиме поражения.
8. **Победа:** иначе `isSolved` → `status = 'completed'`, `result = 'won'`, записать `CompletedGame(won)`, показать `WinScreen`.

Повторная неверная цифра в ту же клетку (после исправления или поверх ошибки) снимает жизнь снова — каждая неверная постановка стоит жизни.

### Режим заметок (toggle)
Тап цифры добавляет/убирает кандидата в выбранной клетке. `Move.wasNote = true`, `clearedNotes` пуст.

### Undo
Снять последний `Move`: откатить `currentGrid[row][col] = prevValue`, восстановить заметки из `clearedNotes`. **Жизни undo НЕ возвращает** — даже если `wasMistake`, потерянная жизнь остаётся потерянной (иначе жизни бессмысленны). Redo нет. На пустом стеке — кнопка disabled.
...
- `Header` — таймер, индикатор жизней (3 «сердца»), Undo / Notes-toggle / New Game, переключатель на статистику, кнопка «Обновить» (когда доступно обновление).
...
- `WinScreen` — финальный экран в двух режимах: победа (со временем) и поражение (кончились жизни), кнопка «Новая игра».
- `InstallPrompt` — Android (`beforeinstallprompt`) + iOS-инструкция (детект Safari, флаг в settings, показ один раз).
- `UpdateBanner` — всплывающий баннер «Доступно обновление»; после закрытия сворачивается в постоянную кнопку «Обновить» в `Header`.
...
2. Показать всплывающий баннер `UpdateBanner` «Доступно обновление» + кнопка «Обновить» (не модалка).
3. Если игрок закрыл баннер — он сворачивается в **постоянную кнопку «Обновить» в `Header`** (флаг `updateReady` в состоянии остаётся true), чтобы обновиться можно было в любой момент.
4. По кнопке «Обновить» (в баннере или хедере): форсировать flush сохранения партии в localStorage → `updateServiceWorker(true)` (skipWaiting + reload).
5. Если игрок так и не обновился — обновление применится при следующем холодном старте. Ничего не ломается.
...
- **Vitest / `state/`:** `gameReducer` (undo с автоочисткой — точный откат; жизни: -1 за ошибку, undo не возвращает жизнь, 0 жизней → `result = 'lost'`), `statsService` (агрегация по периодам, «% завершённых» с учётом `lost`/`abandoned`, «любимая сложность»).
...
- [ ] Undo (полный откат, вкл. заметки; жизни не возвращает), заметки с автоочисткой, таймер с паузой, смена сложности работают без багов.
- [ ] Жизни: старт 3, ошибка (цифра ≠ решению) снимает жизнь, 0 жизней завершает партию поражением; проигрыш отражён в статистике.
