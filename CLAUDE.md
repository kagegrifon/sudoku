...
Версия приложения — [SemVer](https://semver.org/lang/ru/), единственный источник правды — поле
`version` в [package.json](package.json). На экране настроек она показывается автоматически
(подставляется на сборке через `define` в [vite.config.ts](vite.config.ts) — хардкодить версию в коде нельзя).

### Тип изменения

Тип определяется **по влиянию на пользователя**, а не по типу коммита.

| Тип | Когда | Примеры |
|---|---|---|
| **major** (`X.0.0`) | Ломающее изменение для пользователя | Рост `GAME_SCHEMA_VERSION` / `SETTINGS_SCHEMA_VERSION` (сохранённая партия или настройки отбрасываются), удаление фичи, смена привычного поведения |
| **minor** (`0.X.0`) | Новая функциональность без потерь | Новый экран, новая механика, новая настройка |
| **patch** (`0.0.X`) | Всё остальное | Багфиксы, стили, рефакторинг, документация, зависимости, CI |

- **До выпуска 1.0.0 ломающие изменения идут в minor**, а не в major — стандартная практика SemVer для `0.x`.
- Изменение констант `GAME_SCHEMA_VERSION` / `SETTINGS_SCHEMA_VERSION` в
  [src/state/gameTypes.ts](src/state/gameTypes.ts) — **всегда ломающее**: сохранённые данные с чужим
  `schemaVersion` отбрасываются при загрузке.
- Если ветка содержит изменения разных типов — берётся наивысший.

### Чеклист завершения ветки (перед PR)

1. Определить тип изменения по таблице выше.
2. Забампить `version` в [package.json](package.json).
3. Добавить секцию новой версии в [CHANGELOG.md](CHANGELOG.md) — описывать изменения
   с точки зрения пользователя, не по коммитам.
4. Проставить версию в колонку «Заметки» соответствующей строки [docs/roadmap.md](docs/roadmap.md)
   в формате `v0.2.0`.
5. Закоммитить это одним отдельным коммитом: `chore: версия 0.2.0`.

Бамп делается **в фичевой ветке**, не в `main`.

### Проверка

Воркфлоу [.github/workflows/version-check.yml](.github/workflows/version-check.yml) на каждом PR
проверяет, что версия ветки строго больше версии в базовой ветке. Локально то же самое:

```
node scripts/check-version.mjs main
```

Два ограничения, о которых надо помнить:

- Проверка работает только для изменений, вливаемых **через PR**. Коммиты, попадающие в `main`
  напрямую, её не проходят — там правило действует «на честном слове».
- При параллельных ветках возможен конфликт по `version` и `CHANGELOG.md`. Разрешение: ветка,
  вливаемая позже, пересчитывает свой бамп от актуального `main`.
